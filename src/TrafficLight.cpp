...
void TrafficLight::cycleThroughPhases()
{
   ...
   std::random_device rand;
    std::mt19937 rand_en(rand());
    std::uniform_int_distribution<> dist(4000, 6000);
    double cycle_duration = dist(rand_en);
    std::chrono::system_clock::time_point last_update;    

    while(true) {       
        std::this_thread::sleep_for(std::chrono::milliseconds(1));
        auto duration =std::chrono::duration_cast<std::chrono::milliseconds>(std::chrono::system_clock::now() - last_update).count();
        if(duration > cycle_duration) {
            ...
            this->_messages.send(std::move(_currentPhase));
        }
        
        cycle_duration = dist(rand_en);  
